        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              ...
              <button onClick={() => scrollTo('contact')} className="bg-yellow-500 text-slate-900 hover:bg-yellow-400 px-6 py-2 rounded-full text-sm font-bold transition shadow-md">ติดต่อเรา</button>
            </div>
          </div>
...
const Hero = () => (
  <section id="home" className="relative h-screen flex items-center justify-center">
    <div className="absolute inset-0 bg-cover bg-center z-0" style={{ backgroundImage: `url(${DATA.hero.bgImage})` }}>
      <div className="absolute inset-0 bg-slate-900 opacity-70"></div>
    </div>
    <div className="relative z-10 text-center px-4 max-w-5xl mx-auto mt-16">
      <div className="inline-block px-3 py-1 bg-yellow-500 text-slate-900 font-bold rounded-full mb-4 text-sm tracking-wide">PROFESSIONAL SCAFFOLDING & FORMWORK</div>
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-white mb-6 leading-tight drop-shadow-lg">{DATA.hero.title}</h1>
      <p className="text-lg md:text-2xl text-gray-300 mb-10 font-light max-w-3xl mx-auto">{DATA.hero.subtitle}</p>
      <div className="flex flex-col sm:flex-row justify-center gap-4">
        <button onClick={() => document.getElementById('products').scrollIntoView({behavior: 'smooth'})} className="px-8 py-4 bg-yellow-500 text-slate-900 font-bold rounded hover:bg-yellow-400 transition flex items-center justify-center gap-2 text-lg shadow-lg">ดูสินค้าและราคา <ChevronRight size={24} /></button>
        <button onClick={() => document.getElementById('contact').scrollIntoView({behavior: 'smooth'})} className="px-8 py-4 bg-transparent border-2 border-white text-white font-bold rounded hover:bg-white hover:text-slate-900 transition text-lg">ขอใบเสนอราคา</button>
      </div>
    </div>
  </section>
);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="order-2 lg:order-1">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-6 border-l-8 border-yellow-500 pl-6 uppercase tracking-wide">{DATA.about.title}</h2>
          <p className="text-lg text-gray-600 leading-relaxed mb-8">{DATA.about.content}</p>
          <div className="grid grid-cols-2 gap-6 text-center">
            <div className="p-6 bg-slate-50 rounded-xl border border-slate-100"><span className="block text-4xl font-extrabold text-yellow-500 mb-2">10+</span><span className="text-slate-700 font-medium">ปีแห่งประสบการณ์</span></div>
            <div className="p-6 bg-slate-50 rounded-xl border border-slate-100"><span className="block text-4xl font-extrabold text-yellow-500 mb-2">100%</span><span className="text-slate-700 font-medium">QC ตรวจสอบคุณภาพ</span></div>
          </div>
        </div>
...
const Products = () => {
  ...
  return (
    <section id="products" className="py-24 bg-white overflow-hidden">
      <style>{`@keyframes infinite-scroll { from { transform: translateX(0); } to { transform: translateX(-50%); } } .animate-infinite-scroll { animation: infinite-scroll 40s linear infinite; } .animate-infinite-scroll:hover { animation-play-state: paused; }`}</style>
      ...
      <div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
        <div className="flex items-center justify-center md:justify-start [&_li]:mx-8 [&_img]:max-w-none animate-infinite-scroll w-max">
          {loopProducts.map((item, index) => (
            <div key={`${item.id}-${index}`} className="mx-4 w-72 sm:w-80 flex-shrink-0 group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 border border-gray-100 cursor-pointer" onClick={() => document.getElementById('contact').scrollIntoView({behavior: 'smooth'})}>
              <div className="h-56 overflow-hidden relative bg-gray-200"><img src={item.image} alt={item.name} className="w-full h-full object-cover transform group-hover:scale-110 transition duration-700" /></div>
              <div className="p-6 flex flex-col h-auto">
                ...
                <div className="mt-auto">
                  ...
                  <div className="w-full py-2 bg-slate-900 text-white text-sm font-bold rounded-lg group-hover:bg-yellow-500 group-hover:text-slate-900 transition-colors duration-300 flex items-center justify-center gap-2">สอบถามข้อมูล <ChevronRight size={16}/></div>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700 backdrop-blur-sm">
            ...
            <div className="space-y-6">
              <a href={`tel:${DATA.contact.phone.replace(/-/g, '')}`} className="flex items-start gap-4 group cursor-pointer">
                ...
                <div><h4 className="font-bold text-lg text-gray-200 group-hover:text-yellow-500 transition">โทรศัพท์</h4><p className="text-gray-400 text-lg">{DATA.contact.phone}</p></div>
              </a>
              <div className="flex flex-col sm:flex-row gap-4 group bg-slate-900/50 p-4 rounded-xl border border-slate-600/50 hover:border-green-500 transition duration-300">
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div><h4 className="font-bold text-lg text-gray-200 group-hover:text-green-400 transition">Line Official</h4><p className="text-gray-400 text-lg mb-2">{DATA.contact.line}</p><p className="text-xs text-gray-500">สแกน QR Code เพื่อแอดไลน์</p></div>
                </div>
                <div className="flex-shrink-0 bg-white p-2 rounded-lg w-32 h-32 flex items-center justify-center overflow-hidden"><img src={DATA.contact.lineQrImage} alt="Line QR Code" className="w-full h-full object-contain" /></div>
              </div>
              <div className="flex items-start gap-4 group">
                <div className="bg-slate-700 p-3 rounded-xl text-yellow-500"><MapPin size={24} /></div>
                <div><h4 className="font-bold text-lg text-gray-200">ที่อยู่หน้าร้าน</h4><p className="text-gray-400 leading-relaxed">{DATA.contact.address}</p></div>
              </div>
            </div>
          </div>
          <div className="flex gap-4">
            <button className="flex-1 bg-blue-600 py-4 rounded-xl hover:bg-blue-700 transition flex items-center justify-center gap-2 font-bold shadow-lg shadow-blue-900/50"><Facebook /> Facebook</button>
            <button className="flex-1 bg-pink-600 py-4 rounded-xl hover:bg-pink-700 transition flex items-center justify-center gap-2 font-bold shadow-lg shadow-pink-900/50"><Instagram /> Instagram</button>
          </div>
        </div>
        <div className="flex flex-col gap-4">
          <div className="h-[300px] md:h-full bg-slate-800 rounded-2xl overflow-hidden border-2 border-slate-700 shadow-2xl relative group min-h-[300px]">
            <iframe src={DATA.contact.mapEmbedUrl} width="100%" height="100%" style={{border:0}} allowFullScreen="" loading="lazy" title="Map" className="relative z-10 grayscale group-hover:grayscale-0 transition duration-700 ease-in-out"></iframe>
          </div>
          <a href={DATA.contact.mapDirectLink} target="_blank" rel="noopener noreferrer" className="w-full py-4 bg-yellow-500 text-slate-900 font-bold rounded-xl hover:bg-yellow-400 transition shadow-lg flex items-center justify-center gap-3 text-lg"><Navigation size={24} /> เปิดนำทางใน Google Maps</a>
        </div>
      </div>
    </div>
  </section>
...
const Footer = () => (
  <footer className="bg-black text-gray-500 py-10 border-t border-gray-900">
    <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4">
      <div className="flex items-center gap-2 text-yellow-500 font-bold text-xl"><HardHat /> {DATA.brandName}</div>
      <p className="text-sm text-center md:text-right">&copy; {new Date().getFullYear()} {DATA.brandName}. All rights reserved.<br/><span className="text-gray-700 text-xs">Designed for Professional Scaffolding Business</span></p>
    </div>
  </footer>
);
...
export default function App() {
  return (
    <div className="font-sans antialiased text-slate-800 scroll-smooth selection:bg-yellow-500 selection:text-slate-900">
      <Navbar />
      <main><Hero /><Services /><Products /><About /><Contact /></main>
      <Footer />
    </div>
  );
}
